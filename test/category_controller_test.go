...
func setupTestDB() *sql.DB {
	db, err := sql.Open("mysql", "root:123456!@#@tcp(localhost:3306)/belajar-golang-restful-api-test?parseTime=true")
...
func TestCreateCategorySuccess(t *testing.T) {
	...
	requestBody := strings.NewReader(`{"name" : "Monitor"}`)
	request := httptest.NewRequest(http.MethodPost, "http://localhost:3000/api/categories", requestBody)
	request.Header.Add("Content-Type", "application/json")
	request.Header.Add("X-API-Key", "x1234")
	...
	assert.Equal(t, "Monitor", responseBody["data"].(map[string]interface{})["name"])
}
